...
def flatten_prices(currency_code: str, export_df: pd.DataFrame):
    ...
    print(f"Creating flattened version (prices rows -> prices columns)")
    ...
    # Unique product key
    export_df["ProductKey"]=export_df["productName"]  + "\\" + export_df["skuName"] + "\\" + export_df["meterName"] + "\\" + export_df["tierMinimumUnits"].astype(str) + "\\" + export_df["armRegionName"]

    # Extract prices
    prices_df = export_df.loc[:, ["ProductKey","retailPrice","type", "reservationTerm"]]
    ...
    # Reservations - these contains one row per reservation term (1,3 or 5 years) and need to be pivoted ot reservationTerm column
    reservations_df = prices_df.loc[prices_df["type"] == "Reservation"]
    del reservations_df["type"]
    reservations_pivot_df = reservations_df.pivot(index="ProductKey", columns="reservationTerm", values="retailPrice")
    ...
    export_df = pd.merge(export_df, consumption_df, how="left",on=["ProductKey", "ProductKey"])
    export_df = pd.merge(export_df, devtest_df, how="left",on=["ProductKey", "ProductKey"])
    export_df = pd.merge(export_df, reservations_pivot_df, how="left",on=["ProductKey", "ProductKey"])

    # Calculate DevTestConsumption Savings
    col_name = "DevTestConsumption"
    if col_name in export_df:
        export_df[f"{col_name} savings"] = 1 - export_df[col_name] / export_df["Consumption"]
    else:
